...
The game boots to a **main menu**: pick Play, choose one of the three levels
(Warehouse, Facility, Training Range), wait for the loading bar, then click
**Click to Play** — that click is required, because pointer lock and audio
both need a real user gesture.

Assets are generated, not committed by hand:

```
node tools/generateWeaponModels.js   # weapon + projectile .glb
node tools/generateAudio.mjs         # all 62 .wav sounds
node tools/generateSkinTextures.js   # weapon skins
node tools/generateTraversalClips.js # mantle/vault animation clips
```

## Replacing the weapon models

The weapons are procedurally generated by `tools/builders/*.js`. If you want
better-looking models authored elsewhere, hand over
[`WEAPON_MODEL_SPEC.md`](WEAPON_MODEL_SPEC.md) — the complete contract — and
validate whatever comes back with `npm run validate:weapons`.
