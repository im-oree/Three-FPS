...
export class AirstrikeKillstreakController extends KillstreakControllerInterface {
  ...
  override update(dt: number): void {
    ...
    // The jet itself, overhead: one world-positioned flyby rumble just before
    // the first bomb lands, felt even by players clear of the blast line
    // (Document E §1.4 — killstreak events are world-position-based rumbles).
    if (!this.flyby && this.timer >= AIRSTRIKE.INBOUND_DELAY * 0.85) {
      this.flyby = true;
      eventBus.emit('killstreak:airstrike:flyby', {
        x: this.target.x, y: this.target.y + 60, z: this.target.z,
      });
    }

    // CATCH UP rather than dropping at most one bomb per frame. The original
    // returned immediately after each drop, so a single long frame — a GC
    // pause, a shader compile, a throttled tab — stalled the whole stick and
    // only the first bomb ever landed. Drop every bomb whose time has passed.
    while (this.dropped < AIRSTRIKE.BOMB_COUNT) {
      const dropAt = AIRSTRIKE.INBOUND_DELAY
        + this.dropped * AIRSTRIKE.BOMB_INTERVAL;
    ...
    // Every bomb has landed: let the last blast breathe, then finish.
    if (this.timer >= AIRSTRIKE.INBOUND_DELAY
