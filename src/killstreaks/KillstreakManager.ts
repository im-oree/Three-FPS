...
export class KillstreakManager {
   ...
   * so callers (and tests) get a real reason rather than a silent no-op.
   */
  activate(slotIndex: number): string | null {
    const def = this.loadout[slotIndex];
    if (!def) return 'no_such_slot';
    if (!this.deps) return 'not_attached';
    if (this.sinceLastActivation < KILLSTREAK.MIN_ACTIVATION_GAP) return 'too_soon';

    const state = this.stateOf(def.id);
    if (state === 'locked') return 'locked';
    if (state === 'active') return 'already_active';
    if (state === 'cooling') return 'cooling';
    if (this.active.length >= KILLSTREAK.MAX_CONCURRENT) return 'too_many_active';

    const factory = this.factories.get(def.controllerClass);
    if (!factory) return 'no_controller';
