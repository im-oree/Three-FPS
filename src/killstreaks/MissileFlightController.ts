...
export class MissileFlightController {
  ...
  /**
   * Document M §6: the pylon-drop phase. When launched from the jet the
   * missile inherits the jet's velocity and free-falls under gravity until
   * the motor lights — no instant-stop pop, and the classic
   * "drops, tumbles, then the motor lights" ordnance beat.
   */
  private readonly inheritedVelocity: THREE.Vector3;
  private ignitionDelay: number;
  private ignited: boolean;
  private justIgnitedFlag = false;
  ...
  constructor(
    ...
    startYaw: number,
    startPitch: number,
    /** Document M: initial velocity at pylon release (jet's velocity). */
    inheritedVelocity?: THREE.Vector3,
    /** Document M: free-fall seconds before the motor ignites. */
    ignitionDelaySeconds = 0,
  ) {
    this.yaw = startYaw;
    this.pitch = startPitch;
    this.inheritedVelocity = inheritedVelocity?.clone()
      ?? new THREE.Vector3();
    this.ignitionDelay = inheritedVelocity ? ignitionDelaySeconds : 0;
    this.ignited = this.ignitionDelay <= 0;
  ...
  /** True only on the single frame the motor lights (consumed by the owner). */
  get justIgnited(): boolean {
    const j = this.justIgnitedFlag;
    this.justIgnitedFlag = false;
    return j;
  }

  get isIgnited(): boolean { return this.ignited; }

  /**
   * Document M: at ignition the missile adopts a DIVE ONTO THE DESIGNATED
   * POINT, not whatever attitude the inherited jet velocity happened to
   * leave (the tangent at release points up the climbing egress — inheriting
   * it verbatim made the missile sail away and airburst). Designating a
   * point must mean the missile goes there.
   */
  aimAt(point: THREE.Vector3): void {
    const to = _aimScratch.copy(point).sub(this.model.position);
    const planar = Math.hypot(to.x, to.z);
    this.yaw = Math.atan2(-to.x, -to.z);
    this.pitch = THREE.MathUtils.clamp(
      Math.atan2(to.y, planar), MISSILE.MAX_PITCH_DOWN, MISSILE.MAX_PITCH_UP,
    );
    this.applyOrientation(1);
  }
  ...
  update(dt: number, look: { x: number; y: number }, boosting: boolean): MissileStepResult {
    ...
    // -- Free-fall phase (Document M §6.2): inherited jet velocity + gravity,
    // no fuel burn, no steering. A low release can still end things here.
    if (!this.ignited) {
      this.ignitionDelay -= dt;
      this.inheritedVelocity.y -= 9.8 * dt;
      this.prev.copy(this.model.position);
      this.model.position.addScaledVector(this.inheritedVelocity, dt);
      // Nose follows the airstream as the drop steepens.
      const v = this.inheritedVelocity;
      this.yaw = Math.atan2(-v.x, -v.z);
      this.pitch = Math.asin(THREE.MathUtils.clamp(
        v.y / Math.max(1e-4, v.length()), -1, 1,
      ));
      this.applyOrientation(Math.min(1, 8 * dt));
      this.body.setNextKinematicTranslation(this.model.position);
      if (this.ignitionDelay <= 0) {
        this.ignited = true;
        this.justIgnitedFlag = true;
        // Seat the powered-flight attitude on the ACTUAL velocity direction —
        // clamped into the legal cone so a near-vertical droop doesn't stick.
        this.pitch = THREE.MathUtils.clamp(
          this.pitch, MISSILE.MAX_PITCH_DOWN, MISSILE.MAX_PITCH_UP,
        );
        this.applyOrientation(1);
        return { outOfFuel: false, impact: this.sweepForImpact() };
      }
      return { outOfFuel: false, impact: this.sweepForImpact() };
    }

    this.fuel -= dt; // the budget is POWERED flight (Document M §6.2)
