...
import { EFFECTS, VIEWMODEL } from '../utils/Constants';
  ...
  constructor(assetLoader: AssetLoader, private readonly viewmodel: WeaponViewmodel) {
    assetLoader.loadTexture('effects/muzzle_flash.png').then((tex) => {
      ...
      for (let i = 0; i < EFFECTS.POOL_SIZE; i += 1) {
        ...
        // §3.2: the socket (under the camera) lives on the viewmodel layer —
        // the flash must render in the SAME layer-1 pass, never the world pass.
        sprite.layers.set(VIEWMODEL.LAYER_INDEX);
