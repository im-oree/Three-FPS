...
class BallisticsSystem {
  private readonly hittables: HittableEntry[] = [];
  ...
  registerHittable(object: THREE.Object3D, metadata: HittableMetadata): void {
    if (this.hittables.some((h) => h.object === object)) return;
    this.hittables.push({ object, metadata });
  }

  unregisterHittable(object: THREE.Object3D): void {
    const index = this.hittables.findIndex((h) => h.object === object);
    if (index >= 0) this.hittables.splice(index, 1);
  }
