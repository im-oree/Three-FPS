...
export const SOUND_MANIFEST = {
  ...
  // Document M: the guided-missile jet launch cinematic.
  killstreak_jet_approach: 'killstreaks/killstreak_jet_approach.wav',
  killstreak_jet_close: 'killstreaks/killstreak_jet_close.wav',
  killstreak_jet_afterburner_loop: 'killstreaks/killstreak_jet_afterburner_loop.wav',
  missile_release_clunk: 'killstreaks/missile_release_clunk.wav',
  missile_ignition_whoosh: 'killstreaks/missile_ignition_whoosh.wav',
...
export const FOOTSTEP_SETS: Record<string, readonly string[]> = {
  ...
  // Document K surface tags resolve onto the existing recorded sets.
  asphalt: [1, 2, 3, 4].map((i) => `footsteps/footstep_concrete_0${i}.wav`),
  metal_container: [1, 2, 3, 4].map((i) => `footsteps/footstep_metal_0${i}.wav`),
  wood_pallet: [1, 2, 3, 4].map((i) => `footsteps/footstep_wood_0${i}.wav`),
};
...
export const LANDING_SOUNDS: Record<string, string> = {
  ...
  asphalt: 'footsteps/land_concrete.wav',
  metal_container: 'footsteps/land_metal.wav',
  wood_pallet: 'footsteps/land_wood.wav',
};

/** Impact-sound aliases for prop surface tags (Document K). */
const IMPACT_ALIASES: Record<string, string> = {
  asphalt: 'impact_concrete',
  metal_container: 'impact_metal',
  wood_pallet: 'impact_wood',
};
...
export function impactKeyFor(surfaceType: string): string {
  ...
  if (key in SOUND_MANIFEST) return key;
  return IMPACT_ALIASES[surfaceType] ?? 'impact_concrete';
}
