...
export const RECOIL = {
  ...
  /**
   * Document E §2.8 + CoD firing feel: the shot's kick is not rotation-only.
   * The weapon (and the hands riding it) punches STRAIGHT BACK along the
   * barrel a few millimetres and rocks with a hair of signed roll jitter per
   * shot — purely a viewmodel presentation: the aim point, spread and
   * crosshair logic never see any of it.
   */
  VIEWMODEL_KICK_POS_BACK_PER_DEG: 0.0035, // metres backward per kick degree
  VIEWMODEL_KICK_ROLL_JITTER_RAD: 0.006,   // ± signed roll jitter per shot
...
export const POSE_OFFSETS = {
  ...
  /**
   * Killstreak tablet raised: the weapon drops deep to low-ready and out of
   * the read space — deeper than tac sprint because the laptop owns the
   * centre of the frame while it is up (Document I §2.3 / Document J §4.3).
   */
  TABLET: { pos: [0.04, -0.20, 0.08], rotDeg: [-30, 36, 12] },
} as const;
...
export const CAMERA_SHAKE = {
  ...
  /** Killstreak fast-mover overhead: body-rumble felt far from the run line. */
  FLYBY_BASE_TRAUMA: 0.35,
  FLYBY_RADIUS: 45,
} as const;
...
export const TABLET_UI = {
   ...
   */
  CURSOR_GAIN: 0.28,
  /** Safety ceiling on how long the tablet may hold input. */
  MAX_OPEN_SECONDS: 30,
} as const;
...
export const MISSILE = {
  ...
  /**
   * Pitch clamps. The nose may flatten for range but never CLIMB: at +30 deg
   * the old cap let a player level off and run the whole fuel budget out in
   * level flight, which is exactly the "it never reaches the ground, it just
   * explodes in the air" failure. +6 deg keeps it descending throughout.
   */
  MAX_PITCH_UP: 0.1,      // +6 deg — can stretch a dive, cannot climb away
   ...
   * cinematic (JET_CINEMATIC, Document M) owns everything before this.
   */
  HANDOFF_SECONDS: 0.3,
  /** Absolute maximum the streak may own input for (watchdog deadline). */
  CONTROL_HARD_CEILING_SECONDS: 30,
  BLAST_RADIUS: 12,
  BLAST_DAMAGE: 220,
  BOOST_TRAUMA: 0.10,
...
/**
 * JET_CINEMATIC — Document M. The fighter-jet launch sequence that replaced
 * Document I's "spawn above target, cut to nose" launch: the jet enters from
 * airspace, banks down to release altitude, drops the missile, and the
 * camera runs a five-shot sequence before the player takes control.
 *
 * Duration budget: tablet-confirm -> player control ~= 0.62s confirm hold
 * + ~4.3s cinematic = ~5s.
 */
export const JET_CINEMATIC = {
  /** Jet's own spline traversal, secondss — the jet's pacing clock. */
  FLIGHT_DURATION: 5.0,
  /**
   * Shot boundaries (cumulative seconds of the jet clock):
   * establishing 0..1.3 | chase 1.3..2.6 | release+whip-pan 2.6..3.0 |
   * swoop-in 3.0..4.3 | seat & control handoff at 4.3 (missile already
   * ignited at RELEASE + IGNITION_DELAY, so §6.3's "control only after
   * ignition" holds).
   */
  PHASE1_END: 1.3,
  PHASE2_END: 2.6,
  PHASE3_END: 3.0,
  PHASE4_END: 4.3,
  /** Whip-pan look-at transfer inside the release shot. */
  WHIP_SECONDS: 0.25,
  /**
   * Altitude offsets that make cruise -> release -> egress READ as a descent
   * arc, not a flat pass (§5.3). The release altitude itself is
   * MISSILE.LAUNCH_ALTITUDE: the existing missile tuning (flight budget,
   * pitch clamps) is built around that energy state — lowering it would
   * silently kill the player's control time.
   */
  CRUISE_ABOVE_RELEASE: 35,
  EGRESS_ABOVE_RELEASE: 40,
  PAD_RADIUS_FACTOR: 1.1,
  /** Free-fall before the missile's motor lights (§6.2). */
  IGNITION_DELAY_SECONDS: 0.9,
  /** Anti-clipping (§7): raycast margin + minimum ground clearance. */
  ANTI_CLIP_MARGIN: 0.4,
  GROUND_CLEARANCE: 1.2,
  /**
   * Close-pass sound when the jet passes within this distance of the camera
   * (§4: layered flyby rumble, same pattern as the airstrike's).
   */
  CLOSE_PASS_DISTANCE: 45,
  /**
   * Conservative bounding sphere for the path validator's clip checks (§8.1).
   * Mirrors fighter_jet.glb's userData.boundingRadius (JetBuilder).
   */
  BOUNDING_RADIUS: 5.8,
} as const;
