...
export class Debug {
  ...
  private readonly lineProviders = new Map<string, () => string>();
  ...
  /**
   * Register a named live readout line (e.g. stamina). Generic seam so any
   * system can surface diagnostics without touching this file's internals.
   */
  addLine(id: string, provider: () => string): void {
    this.lineProviders.set(id, provider);
  }

  removeLine(id: string): void {
    this.lineProviders.delete(id);
  }
  ...
  update(dt: number): void {
    ...
    this.element.textContent =
      ...
      (this.lineProviders.size
        ? `\n${[...this.lineProviders.values()].map((provider) => provider()).join('\n')}`
        : '');
  }
