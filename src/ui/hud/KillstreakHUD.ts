...
export class KillstreakHUD {
  ...
  constructor(private readonly manager: KillstreakManager) {
    ...
    // Activation refusals flash the offending slot red — pressing a killstreak
    // button must never be met with silence. The class retriggers its
    // animation via the offsetWidth reflow trick.
    eventBus.on('killstreak:denied', (payload) => {
      const { id } = payload as { id: string };
      const index = this.manager.snapshot().findIndex((s) => s.id === id);
      const el = this.slots[index];
      if (!el) return;
      el.root.classList.remove('ks-slot--denied');
      void el.root.offsetWidth;
      el.root.classList.add('ks-slot--denied');
      window.setTimeout(() => el.root.classList.remove('ks-slot--denied'), 500);
    });
    for (let i = 0; i < 3; i += 1) {
