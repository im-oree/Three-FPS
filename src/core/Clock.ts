...
export class Clock {
   ...
   */
  /**
   * Fraction (0..1) of one fixed step still remaining in the accumulator —
   * the interpolation alpha render-side visuals need to avoid 60 Hz
   * stair-stepping on high-refresh displays (Document 3 feel fix).
   */
  getFixedAlpha(fixedDt: number): number {
    return Math.min(1, this.fixedStepAccumulator / fixedDt);
  }
