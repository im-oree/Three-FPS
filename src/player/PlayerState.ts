...
export function resolveNextState(
  ...
  if (!physics.isGrounded) {
    // JUMP holds through the whole ASCENT (vy > 0); descent resolves AIR.
    // Without this the jump press's one-frame edge collapsed JUMP to a
    // single frame and the state was unobservable (Doc 2: JUMP = ascent).
    if (current === PlayerState.JUMP && physics.verticalVelocity > 0) return PlayerState.JUMP;
