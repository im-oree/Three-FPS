...
export class PlayerHealth {
  ...
  takeDamage(amount: number, sourceWorldPosition?: THREE.Vector3): void {
    ...
    // God-mode cheat (main menu CHEATS panel): damage still registers — the
    // vignette, direction indicator and camera trauma all stay real — but the
    // floor is 1 HP, so `player:died` can never fire while the toggle is on.
    if (this.current === 0 && cheatsStore.get(CheatId.GOD_MODE)) this.current = 1;
    this.regenTimer = 0;
