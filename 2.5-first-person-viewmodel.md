> **SUPERSEDED** — replaced by `A-procedural-rig.md` + `B-procedural-melee-polish.md`
> (the procedural low-poly rigid-joint rig). The rendering pipeline, movement,
> combat and socket contracts below remain valid where the new documents say
> "unchanged/recap"; everything hands/arms/weapon-geometry related here is
> discarded.

# Document 2.5 — FIRST-PERSON CONTROLLER & VIEWMODEL SYSTEM

> **Status: AUTHORITATIVE.** This document supersedes Document 2's camera/weapon-adjacent
> sections and Document 3's viewmodel/animation sections wherever they conflict.
> Committed verbatim as the implementation contract for the first-person body.
>
> Implementation status of each section is tracked in `tools/verify/acceptance-viewmodel.mjs`.
...
- Everything else from Documents 1, 4, 5, 6 (engine, levels, UI, audio) remains as previously specified and is assumed to exist. Where this document changes a file's responsibility, that file's earlier spec is considered **superseded**, not additive.
...
```
MainCamera (THREE.PerspectiveCamera, driven by PlayerCamera)
...
1. **Base Locomotion Layer** — a single baked clip selected by current `PlayerState` movement state (`idle`, `walk`, `sprint`, `crouch_idle`, `crouch_walk`, `slide`, `jump_start/loop/land`), crossfaded between on transition (Document 3's `AnimationBlender.crossfadeTo`).
