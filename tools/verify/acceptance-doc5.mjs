...
// SPLIT into separate evaluates. Frame-accurate waits are the only reliable
// way to observe render-loop state, but headless runs rAF at ~24 ms, so
// batching the whole section into one call overran the CDP protocol timeout
// and died with an error that looked like a game hang.
const crosshair = await page.evaluate(async () => {
  ...
  for (let i = 0; i < 90; i += 1) {
    ...
    if (O.playerController.getState() === 'SPRINT') break;
  }
  for (let i = 0; i < 12; i += 1) await new Promise((r) => requestAnimationFrame(r));
  const gapMoving = O.hud.crosshairGap;
  O.inputManager.heldKeys.clear();
  for (let i = 0; i < 24; i += 1) await new Promise((r) => requestAnimationFrame(r));
  return { gapIdle, gapMoving };
});

await page.evaluate((c) => { window.__crosshair = c; }, crosshair);
const hudLive = await page.evaluate(async () => {
  const O = window.__OPERATOR__;
  const sleepFor = (ms) => new Promise((r) => setTimeout(r, ms));
  const { gapIdle, gapMoving } = window.__crosshair;
  ...
  // The marker's lifetime is measured in SIM time and cleared in the update
  // loop, so both samples must be taken on frames.
  ...
  // Frames, not wall-clock: rounds are consumed in the update loop.
...
const pause = await page.evaluate(async () => {
  ...
  // Hold the key through the sample window: the check is that the sim RESUMED,
  // so the player must actually be asked to move during it.
  O.inputManager.heldKeys.add('KeyW');
  for (let i = 0; i < 70; i += 1) await new Promise((r) => requestAnimationFrame(r));
