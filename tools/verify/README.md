...
provisions and removes its own temporary texture fixture. **Historical:** the
Document 2 spec required deleting Doc-1's TEMP console proofs, so several of
its assertions no longer apply to the current `main.ts`; it is kept as the
record of Doc-1's passing gate (11/11 at commit `73924df`).

`acceptance-doc2.mjs` automates the Document 2 §14 checklist (31 checks) by
actually *playing* the game: real CDP keyboard events drive WASD/sprint/
crouch/jump, synthetic pointer-lock `MouseEvent`s drive look (CDP mouse moves
carry no `movementX/Y` while pointer-locked in headless — a documented
limitation; the game listener is trust-agnostic), and three TEST-ONLY seams on
`PlayerController` (`debugTeleport`, `debugSetStamina`, `debugSetOrientation`,
each doc-commented as harness-only) make scenarios deterministic. Timing
notes for anyone extending it:

- Headless rAF is throttled (~10-20 fps here) and `CLOCK.MAX_DELTA` clamping
  makes sim time run slower than wall time, so all assertions are either
  state-based, use in-page `setInterval` recorders on the page's own clock,
  position gates (`waitForFunction` on coordinates), or are anchored to the
  first relevant sample. Wall-clock sleeps are never load-bearing.
- A key tap synthesised *inside* a game `update()` callback is swallowed by
  `InputManager`'s deferred-release latch (it is cleared by that same frame's
  `endFrame()` before the next update can see the pressed edge); dispatch such
  taps from a `setTimeout(…, 0)` so they land between frames like real input.
- The run disables shadow maps and halves pixel ratio for timing stability,
  restoring both before the final screenshot.

Each later document adds its own `acceptance-docN.mjs` the same way.
