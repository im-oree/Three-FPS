...
// Document M §4: the guided-missile jet launch cinematic -------------------
// Approach: 7s of turbine whine + filtered noise rising from far to close —
// begins well before the jet is in frame, matching real approach acoustics.
w('killstreaks/killstreak_jet_approach.wav', normalize(render(7.0, (t) => {
  const easeIn = Math.min(1, t / 6.2) ** 1.6;
  const whine = Math.sin(2 * Math.PI * (900 + 900 * easeIn) * t) * 0.5
    + Math.sin(2 * Math.PI * (1350 + 1500 * easeIn) * t) * 0.22;
  const roar = noise() * (0.25 + 0.65 * easeIn);
  return (whine * 0.55 + roar * 0.45) * (0.12 + 0.88 * easeIn);
}), 0.62));
// Close pass: a 2s full-throated roar burst layered in as the jet goes by.
w('killstreaks/killstreak_jet_close.wav', normalize(render(2.0, (t) => {
  const env = Math.sin(Math.PI * Math.min(1, t / 1.9)) ** 1.4;
  let lp = 0;
  const n = noise();
  lp = lp + 0.35 * (n - lp);
  return (lp * 1.9 + Math.sin(2 * Math.PI * 240 * t) * 0.4) * env;
}), 0.78));
// Afterburner loop: seamless turbine bed that follows the airframe.
w('killstreaks/killstreak_jet_afterburner_loop.wav', ambience({ secs: 5,
  base: 96, noiseLevel: 0.34, cutoff: 1400, partials: [1, 1.97, 3.02, 4.03] }));
// Pylon release: a short metallic clunk + spring ring — the rack letting go.
w('killstreaks/missile_release_clunk.wav', normalize(render(0.38, (t) => {
  const thump = Math.sin(2 * Math.PI * 210 * t) * decay(t, 0.30, 2.8);
  const ring = Math.sin(2 * Math.PI * 1240 * t) * decay(t, 0.10, 5.5) * 0.5;
  const clank = noise() * decay(t, 0.055, 4.5) * 0.7;
  return thump * 0.85 + ring + clank;
}), 0.8));
// Motor ignition: a 1.3s rising whoosh of fresh-burn noise.
w('killstreaks/missile_ignition_whoosh.wav', normalize(render(1.3, (t) => {
  const env = hit(t, 0.07, 1.2, 2.2);
  let lp = 0;
  lp = lp + Math.min(1, (900 + 2600 * t) / (RATE / 2)) * (noise() - lp);
  return (lp * 2.1 + Math.sin(2 * Math.PI * (160 + 320 * t) * t) * 0.35) * env;
}), 0.75));
